...
public class OpcUaClientSingleton {
    ...
    public static synchronized OpcUaClient getInstance() throws Exception {
        if (instance == null) {
            List<EndpointDescription> endpoints = DiscoveryClient.getEndpoints("opc.tcp://127.0.0.1:4334").get();
            EndpointDescription configPoint = EndpointUtil.updateUrl(endpoints.get(0), "127.0.0.1", 4334);
